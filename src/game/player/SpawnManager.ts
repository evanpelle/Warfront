 ...
 */
export function spawnPoints(gs: GameState, maxPlayers: number): number[] {
	let radius = Math.max(5, Math.sqrt(gameMap.width * gameMap.height / maxPlayers / 1.1 / Math.sqrt(2)));
	while (radius >= 5) {
		var spawnPoints = buildSpawns(radius);
...
function randomSpawnPoint(player: Player): number {
	...
	getSpawnPixels(result).forEach(pixel => this.gs.conquer(pixel, player.id));
	return result;
}
 ...
 * @private
 */
function getSpawnPixels(tile: number): number[] {
	let result = [];
